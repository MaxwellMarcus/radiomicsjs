 ...
 */
async function writeLocalFile(useCompression, imageOrMesh, filePath) {
    const absoluteFilePath = path.resolve(filePath);
    const extension = getFileExtension(absoluteFilePath);
    const isMesh = extensionToMeshIO.has(extension);
    if (isMesh) {
        try {
            await writeMeshLocalFile({ useCompression }, imageOrMesh, filePath);
            return null;
        }
        catch (err) {
            // Was a .vtk image file? Continue to write as an image.
            await writeImageLocalFile(useCompression, imageOrMesh, filePath);
            return null;
        }
    }
    else {
